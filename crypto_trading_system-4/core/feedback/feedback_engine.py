...
class FeedbackEngine:
    ...
    def record_trade(self, trade: TradeRecord) -> bool:
        ...
        try:
            ...
            # 检查是否需要触发分析
            if len(self.performance_analyzer.trades) % 10 == 0:
    ...
    def close_trade(self, 
        ...
        """
        try:
            success = self.performance_analyzer.close_trade(
            ...
            if success:
                # 创建学习样本
                trade = next(
                    (t for t in self.performance_analyzer.trades if t.id == trade_id),
                    None
                )
                if trade and trade.result != TradeResult.OPEN:
                    sample = LearningSample(
                        state={},  # 可以从决策上下文获取
                        action=trade.entry_side,
    ...
    async def analyze_performance(self, 
        ...
        try:
            ...
            # 生成洞察
            insights = self.performance_analyzer.generate_insights(metrics)
            ...
            # 保存到存储
            self.feedback_store.store_performance_metrics(metrics)
    ...
    async def run_online_learning(self) -> FeedbackResult:
        ...
        try:
            ...
            # 执行学习
            updates = self.online_learner.learn()
            
            # 应用更新
            applied_updates = []
            if self.config['auto_apply_updates'] and self.mode == FeedbackMode.AUTO:
            ...
            # 保存状态
            self.online_learner.save_state()
    ...
    def _apply_model_update(self, update: ModelUpdate) -> bool:
        ...
        try:
            # 这里可以实现具体的更新应用逻辑
            logger.info(f"应用模型更新: {update.component} -> {update.new_value}")
            return True
        except Exception as e:
    ...
    def add_human_feedback(self, feedback: HumanFeedback) -> bool:
        ...
        try:
            ...
            self.feedback_store.store_human_feedback(feedback)
    ...
    async def train_rlhf(self) -> FeedbackResult:
        ...
        try:
            ...
            # 训练奖励模型
            reward_metrics = self.rlhf_trainer.train_reward_model(epochs=10)
            
            # 优化策略
            policy_metrics = self.rlhf_trainer.optimize_policy(epochs=10)
            
            # 保存模型
            self.rlhf_trainer.save_models()
    ...
    def generate_report(self, 
        ...
        # 代理性能
        agent_performances = self.online_learner.get_agent_performances()
        ...
        report = {
            ...
            'agents': {
                name: {
                    ...
                    'current_weight': self.online_learner.agent_weights.get(name, 0.0)
                }
                for name, perf in agent_performances.items()
            },
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'trades_count': len(self.performance_analyzer.trades),
            'samples_count': len(self.online_learner.samples),
            'feedback_count': len(self.rlhf_trainer.human_feedback),
            'agent_weights': self.online_learner.agent_weights,
            'confidence_threshold': self.online_learner.confidence_threshold
        }
